...
const USE_VOLUME_FILTER = false;  // Disabled — don't downgrade signals on volume
...
function runStrategy(
  ...
  if (mostRecentCrossIdx >= 0) {
    ...
    if (isMostRecentBullish) {
      ...
      // Volume noted but don't downgrade signal
      if (signal === "BUY" && !volumeCondition) {
        positionStatus = positionStatus === "ACTIVE" ? "ACTIVE_LOW_VOL" : "WAIT_VOL";
      }
    ...
    // No fresh crossover — generate signals based on EMA alignment + momentum
    const ema5 = ema9Arr[lastIdx];
    const ema13 = ema9Arr[Math.max(0, lastIdx - 4)];
    const priceMomentum = closes[lastIdx] - closes[Math.max(0, lastIdx - 3)];
    const rsiVal = calcRSI(closes, RSI_LENGTH);
    
    if (ema9 > ema21 && trendStrength > 0.05) {
      // Bullish EMA alignment — generate BUY
      const atrVal = atrSeries[atrSeries.length - 1] || 0;
      const entryVal = closes[lastIdx];
      const slCalc = entryVal - (atrVal * ATR_MULT_STOPLOSS);
      const tp1Calc = entryVal + (atrVal * ATR_MULT_TARGET * 0.5);
      const tp2Calc = entryVal + (atrVal * ATR_MULT_TARGET);
      const tp3Calc = entryVal + (atrVal * TARGET3_MULT);
      
      signal = "BUY";
      entryPrice = Math.round(entryVal * 100) / 100;
      slPrice = Math.round(slCalc * 100) / 100;
      tp1 = Math.round(tp1Calc * 100) / 100;
      tp2 = Math.round(tp2Calc * 100) / 100;
      tp3 = Math.round(tp3Calc * 100) / 100;
      ...
      buyScore = 40 + Math.round(trendStrength * 2);
      if (volumeSurge) buyScore += 15;
      else if (volumeCondition) buyScore += 10;
      if (rsiVal < RSI_OVERSOLD) buyScore += 10;
      if (price > calcVWAP(closes, volumes)) buyScore += 5;
      if (candlePattern.type === "bullish") buyScore += 10;
      if (srConfirm.confirmed) buyScore += 10;
      buyScore = Math.min(100, buyScore);
      sellScore = Math.max(0, 20 - Math.round(trendStrength));
    } else if (ema9 < ema21 && trendStrength > 0.05) {
      // Bearish EMA alignment — generate SELL
      const atrVal = atrSeries[atrSeries.length - 1] || 0;
      const entryVal = closes[lastIdx];
      const slCalc = entryVal + (atrVal * ATR_MULT_STOPLOSS);
      const tp1Calc = entryVal - (atrVal * ATR_MULT_TARGET * 0.5);
      ...
      sellScore = 40 + Math.round(trendStrength * 2);
      if (volumeSurge) sellScore += 15;
      else if (volumeCondition) sellScore += 10;
      if (rsiVal > RSI_OVERBOUGHT) sellScore += 10;
      if (price < calcVWAP(closes, volumes)) sellScore += 5;
      if (candlePattern.type === "bearish") sellScore += 10;
      if (srConfirm.confirmed) sellScore += 10;
      sellScore = Math.min(100, sellScore);
      ...
      symbols.map(async (sym: string) => {
        ...
        try {
          ...
          return {
            ...
            previousClose: prev,
            signalTime: meta.regularMarketTime ? new Date(meta.regularMarketTime * 1000).toLocaleTimeString('en-IN', {timeZone:'Asia/Kolkata', hour12:false, hour:'2-digit', minute:'2-digit', second:'2-digit'}) + ' IST' : new Date().toLocaleTimeString('en-IN', {timeZone:'Asia/Kolkata', hour12:false, hour:'2-digit', minute:'2-digit', second:'2-digit'}) + ' IST',
            error: false
          };
        } catch (yahooErr: any) {
          ...
          try {
            ...
            return {
              ...
              signalTime: new Date().toLocaleTimeString('en-IN', {timeZone:'Asia/Kolkata', hour12:false, hour:'2-digit', minute:'2-digit', second:'2-digit'}) + ' IST',
