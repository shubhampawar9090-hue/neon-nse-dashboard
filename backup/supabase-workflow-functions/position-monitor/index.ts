...
Broadcast this to the owner. Do not perform any other actions.` }),
  ...
  try {
    ...
    for (let i = 0; i < iterations && activeTrades.length > 0; i++) {
      ...
      for (const trade of [...activeTrades]) {
        ...
        const pnl = Math.round((ltp - trade.entryPrice) * trade.qty * 100) / 100;
        
        if (shouldExit) {
          const order = await placeSellOrder(trade);
...
Deno.serve(async (req) => {
  ...
  if (trades.length === 0) {
    const rows = await dbList("ai_trades?execution_status=eq.OPEN&qty=gt.0&select=id,symbol,qty,entry_price,peak_ltp,trailing_stop,sl,tp1,tp2,tp3,signal,instrument_token");
  ...
  if (data?.results && Array.isArray(data.results)) {
    ...
    if (exits.length > 0) {
      notifyAgent(`Position exits detected by Supabase position-monitor:\n\n${JSON.stringify(exits, null, 2)}\n\nBroadcast the exit details to the owner (symbol, exit reason, P&L).`);
    }
  }
