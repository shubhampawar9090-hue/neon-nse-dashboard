...
function expiryToDate(expiryCode: string): string | null {
  const monthMap: Record<string, number> = { JAN: 0, FEB: 1, MAR: 2, APR: 3, MAY: 4, JUN: 5, JUL: 6, AUG: 7, SEP: 8, OCT: 9, NOV: 10, DEC: 11 };
  const m = expiryCode.match(/^(\d{2})([A-Z]{3})$/);
  if (!m) return null;
  const day = parseInt(m[1]); const month = monthMap[m[2]];
  if (month === undefined) return null;
  const now = new Date(); let year = now.getFullYear();
  let d = new Date(year, month, day, 15, 30, 0);
  if (d < now) d = new Date(year + 1, month, day, 15, 30, 0);
  ...
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (optMatch) {
        ...
        optionChain[underlying].push({ symbol: sym, token, strike: parseInt(strike), type, expiry, underlying, expiryDate: expiryToDate(expiry) });
      } else {
        // numeric weekly format e.g. NIFTY2690823950CE = yy + month(1-9/O/N/D) + dd + strike
        const wkMatch = sym.match(/^(NIFTY|BANKNIFTY|SENSEX)\d{2}(\d|[OND])(\d{2})(\d+)(CE|PE)$/);
        if (wkMatch) {
          const [, underlying, mChar, day, strike, type] = wkMatch;
          const monNames: Record<string, string> = { '1': 'JAN', '2': 'FEB', '3': 'MAR', '4': 'APR', '5': 'MAY', '6': 'JUN', '7': 'JUL', '8': 'AUG', '9': 'SEP', 'O': 'OCT', 'N': 'NOV', 'D': 'DEC' };
          const expCode = `${day}${monNames[mChar] || 'DEC'}`;
          if (!optionChain[underlying]) optionChain[underlying] = [];
          optionChain[underlying].push({ symbol: sym, token, strike: parseInt(strike), type, expiry: expCode, underlying, expiryDate: expiryToDate(expCode) });
        }
      }
    }
...
function getNearestExpiry(chain: any[]) {
  ...
  for (const opt of chain) (byExp[opt.expiry] = byExp[opt.expiry] || []).push(opt);
  const cutoff = Date.now() - 23 * 3600 * 1000; // expiry day itself still tradeable
  let best = ''; let bestTs = Infinity;
  for (const [exp, opts] of Object.entries(byExp)) {
    const ts = opts[0].expiryDate ? new Date(opts[0].expiryDate).getTime() : NaN;
    if (!isNaN(ts) && ts >= cutoff && ts < bestTs) { bestTs = ts; best = exp; }
  }
  if (!best) { // fallback: legacy most-contracts heuristic
    let max = 0;
    for (const [exp, c] of Object.entries(byExp)) if (c.length > max) { max = c.length; best = exp; }
  }
  return best;
}
...
function parseOptionSignals(taData: any, priceData: any[], chain: any[], vixValue: number) {
  const indexMap: Record<string, { underlying: string; stepSize: number }> = {
    ...
    '^BSESN': { underlying: 'SENSEX', stepSize: 100 },
  };
  const results: any[] = [];
  for (const ta of (taData.data || [])) {
    ...
    results.push({
      ...
      tradingSymbol: atmWithGreeks?.symbol || atmOpt.symbol, instrumentToken: atmWithGreeks?.token || atmOpt.token,
      strike: atmOpt.strike, type: optType, premium: atmWithGreeks?.greeks?.theoreticalPremium ?? null,
